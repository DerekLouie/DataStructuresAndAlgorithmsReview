...
var sorted, sortingAlgo, failed = [], passed;

var document = document || 0;

for (sortingAlgo in sorter) {
    testingArray.forEach(
            function(element,index){
                sorted = sorter[sortingAlgo](element.slice(0));
                passed = true;
                // console.log("Test "+ index +": ("+sortingAlgo +"): \n","Original: ", element, "\n   Answer: ", sorted);
                if(sorted.toString() !== answer.toString()) {
                    console.log("TEST FAILED"); 
                    failed.push("Test "+ index +": ("+ sortingAlgo +")");
                    passed = false;
                }
            });
    if (passed) {
        console.log("TEST "+sortingAlgo+": PASSED"); 
        if(document) {
            document.write("TEST "+sortingAlgo+": <span class='passed'>PASSED</span><br/>")
        }
    }
} 
if(failed.length){
